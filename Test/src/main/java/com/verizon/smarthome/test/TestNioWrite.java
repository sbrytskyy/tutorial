...
public class TestNioWrite {
	 ...
	 *
	 * @throws java.io.IOException
	 *             if problems writing bytes.
	 */
	private static void writeRawBytes() throws IOException {
		final FileOutputStream fos = new FileOutputStream("/Users/brytsse/temp/tempout.txt");
		// allocate a channel to write that file
		FileChannel fc = fos.getChannel();
		// allocate a buffer, as big a chunk as we are willing to handle at a
		// pop.
		// Unlike the buffer on a stream, item is up to you not to overflow the
		// buffer.
